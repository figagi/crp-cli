...
class Service {
  ...
  init(params) {
    ...
    //   const cdnDomain = this.env === 'prod' ? qiniuConfig.CDN_DOMAIN_PEOD : qiniuConfig.CDN_DOMAIN_DEV;
    //   process.env.PUBLIC_URL = `${cdnDomain}/${config.name}-${process.env.FIG_ENV}`
    // }
  ...
  loadConfig() {
    const { loadConfig } = require(`${privateName}/dev-utils`)
    return loadConfig() || {};
  }
}
